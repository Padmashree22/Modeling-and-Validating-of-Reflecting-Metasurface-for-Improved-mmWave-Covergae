% Author: Padmashree Reddy Malur Vemana
% Masters in Automation and Robotics
% Affiliation: TU Dortmund, Germany 

% Clear command window, close all figures, and clear workspace
clc; 
close all; 
clear all;

% Add path to helper function directory
addpath('Helper_function') % load helper_function

% Load pre-defined data
load('LOS_scenario1_grid', 'LOS_grid')
load('NLOS_region')

%% Define simulation parameters
freq = 28e9;  % Frequency in Hz (28 GHz)
c = physconst('Light');  % Speed of light
tx_position = [309.9, 170.1, 10.0];  % Transmitter position [x, y, z] in meters 
reflector_center = [195, 170, 5];  % Reflector footprint [x, y, z] in meters
reflector_size = [3, 3];  % [length, width] in meters
h_E = 1;          % Effective environment height

P_TX = 20.3; % dBm
G_TX = 19.7; % dBi
G_RX = 13.5; % dBi
R_Sensitivity = 83.5; %dBm

% Grid receiver
receiver_x = 120:1:240;
receiver_y = 120:1:240;
receiver_z = 1.5;
[X, Y] = meshgrid(receiver_x, receiver_y);
Y = rot90(Y, 2);

phi_in = 80.17; % Incident azimuth angle [deg]
theta_in = -2.46; % Incident elevation angle [deg]

%% Pattern Resolution and Angle Ranges
pattern_resolution = 0.1; % Resolution of RCS [deg]
phi_out_min = -90; % Minimum reflected azimuth wave [deg] 
phi_out_max = 90; % Maximum reflected azimuth wave [deg]
theta_out_min = -90; % Minimum reflected elevation wave [deg]
theta_out_max = 90; % Maximum reflected elevation wave [deg]

%% IRS Array Configuration
N = 1:32; % Modules along Y axis 
M = 1; % Modules along Z axis
a_dim = 3 / 32; % Dimensions along Y axis [m]
b_dim = 3; % Dimensions along Z axis [m]

%% Slope angle variation: Narrow and Broad
alpha_var = 27.6;
beta_var = 0;
var = 2.5;

alpha = (alpha_var) .* ones(length(M), length(N)); % Horizontal Slope angle [deg]
alpha = flip(alpha);

beta = (beta_var) .* ones(length(M), length(N)); % Vertical Slope angle [deg]
beta = flip(beta);

%% Reflected Wave Angle Ranges
theta_out = theta_out_min:pattern_resolution:theta_out_max; % Reflected elevation wave [deg]
phi_out = phi_out_min:pattern_resolution:phi_out_max; % Reflected azimuth wave [deg]
lambda = c / freq; % Wavelength [m]

%% Distances
distance_tx_rx = sqrt((X - tx_position(1)).^2 + (Y - tx_position(2)).^2 + (receiver_z - tx_position(3)).^2); % Tx-Rx distance
distance_tx_ref = sqrt((reflector_center(1) - tx_position(1)).^2 + (reflector_center(2) - tx_position(2)).^2 + (reflector_center(3) - tx_position(3)).^2); % Tx-Reflector distance
distance_ref_rx = sqrt((X - reflector_center(1)).^2 + (Y - reflector_center(2)).^2 + (receiver_z - reflector_center(3)).^2); % Rx-Reflector distance

FF_dist = max(((a_dim / 10).^2), ((b_dim / 10).^2)) / lambda; % Far-field distance

%% Gain_RCS Calculation
[sigma_sum_max, sigma_sum_mat, eta_a, eta_b] = fn_SF_HELIOS_array_eff(a_dim, b_dim, N, M, alpha, beta, phi_in, theta_in, phi_out, theta_out, lambda);
sigma_sum_mat = (10 .* log10(sigma_sum_mat.^2));

%% LOS and NLOS Grid Classification
for f = 1:length(receiver_y)
    for g = 1:length(receiver_x)
        if receiver_y(f) <= 190 && receiver_y(f) >= 170
            LOS_grid(f, g) = 1;
        elseif (receiver_y(f) >= 190 && (receiver_x(g) < 170 || receiver_x(g) > 190)) || (receiver_y(f) <= 170 && (receiver_x(g) <= 170 || receiver_x(g) > 190))
            LOS_grid(f, g) = 2;
        end
    end
end

%% Path Loss and Power Calculations
[PL_LOS_dB, PL_NLOS_dB, PL_Radar_dB] = Fn_FSPL(freq.' * 1e-9, (distance_tx_ref), (distance_ref_rx), (distance_tx_rx), c, distance_tx_ref);

for f = 1:length(receiver_y)
    for g = 1:length(receiver_x)
        if (receiver_y(f) <= 190 && receiver_y(f) >= 170) || (receiver_x(g) < 190 && receiver_x(g) > 170) 
            if LOS_grid(f, g) == 1      %% LOS
                PL1(f, g) = PL_LOS_dB(f, g);
                P_RX1(f, g) = P_TX + G_TX + G_RX - PL1(f, g);
            else
                PL1(f, g) = -Inf; 
                P_RX1(f, g) = -Inf;
                gain_rcs1(f, g) = -Inf;
            end
        else
            gain_rcs1(f, g) = NaN;
            PL1(f, g) = NaN; 
            P_RX1(f, g) = NaN;
        end
    end
end

for f = 1:length(receiver_y)
    for g = 1:length(receiver_x)
        if (receiver_y(f) <= 190 && receiver_y(f) >= 170) || (receiver_x(g) < 190 && receiver_x(g) > 170) 
            if (receiver_y(f) <= 190 && receiver_y(f) >= 170) || (receiver_x(g) < 190 && receiver_x(g) > 170 && receiver_y(f) <= 190)        %% LOS  %% LOS
                PL2(f, g) = PL_Radar_dB(f, g);
                gain_rcs2(f, g) = sigma_sum_mat(a1(f, g) + 1, a2(f, g) + 1);
                P_RX2(f, g) = P_TX + G_TX + G_RX + gain_rcs2(f, g) - PL2(f, g);
            else  
                gain_rcs2(f, g) = -Inf;
                PL2(f, g) = -Inf; 
                P_RX2(f, g) = -Inf;
            end
        else
            gain_rcs2(f, g) = NaN;
            PL2(f, g) = NaN; 
            P_RX2(f, g) = NaN;
        end
    end
end

% Combine received power from LOS and Radar
Combined_P_RX = max(P_RX1, P_RX2);

% Calculate power difference and handle special cases
for f = 1:length(receiver_y)
    for g = 1:length(receiver_x)
        if (receiver_y(f) <= 190 && receiver_y(f) >= 170) || (receiver_x(g) < 190 && receiver_x(g) > 170) 
            if (receiver_y(f) <= 190 && receiver_y(f) >= 170) || (receiver_x(g) < 190 && receiver_x(g) > 170 && receiver_y(f) <= 190)        %% LOS  %% LOS
                if LOS_grid(f, g) == 1
                    delta_power(f, g) = P_RX2(f, g) - P_RX1(f, g);
                elseif LOS_grid(f, g) == 0
                    delta_power(f, g) = P_RX2(f, g) + 100;
                end
                if distance_ref_rx(f, g) <= FF_dist
                    delta_power(f, g) = -Inf;
                    gain_rcs2(f, g) = -Inf;
                    Combined_P_RX(f, g) = -Inf;
                end
            else
                delta_power(f, g) = -Inf;
            end
        else
            delta_power(f, g) = NaN;
        end
        if reflector_center(1) == 195 && NLOS_region_case_sens(f, g) == 1
            delta_power(f, g) = -Inf;
            gain_rcs2(f, g) = -Inf; 
            Combined_P_RX(f, g) = -Inf; 
        end
    end
end

% Plot the results
figure;
subplot(1, 2, 1);
plot_heatmap_analytical(gain_rcs2, receiver_x, receiver_y, ...
                 'Gain [dB]', 'X-position [m]', 'Y-position [m]', ...
                 -30, 60) 
subplot(1, 2, 2);
plot_heatmap_analytical(Combined_P_RX, receiver_x, receiver_y, ...
                 'Maximum Power Received [dBm]', 'X-position [m]', 'Y-position [m]', ...
                 -130, -10)
